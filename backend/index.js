...
app.get('/api/records/:id/pdf', authMiddleware, async (req, res) => {
  try {
    const recordId = req.params.id;
    const patient = await Patient.findById(recordId);
    if (!patient) {
    ...
    doc.on('end', () => {
      const pdfBuffer = Buffer.concat(chunks);
      res.setHeader('Content-Type', 'application/pdf');
      res.setHeader('Content-Disposition', `attachment; filename=Record_${patient.name}.pdf`);
      res.send(pdfBuffer);
    });

    doc.fontSize(18).text('Patient Record', { align: 'center' }).moveDown();
    doc.fontSize(12).text(`Name: ${patient.name}`);
    doc.text(`Email: ${patient.email}`);
    doc.text(`Age: ${patient.age}`);
    doc.text(`Gender: ${patient.gender}`);
    doc.text(`Date: ${new Date(patient.createdAt).toLocaleString()}`);

    if (isDoctor && patient.results) {
      doc.moveDown().text('Diagnosis Results:');
      Object.entries(patient.results).forEach(([k, v]) => doc.text(`${k}: ${v}`));
    }

    if (patient.prescription) {
      doc.moveDown().text('Prescription:');
      doc.text(patient.prescription);
    }
